<?php 
global $the_query, $load_posts, $wp_the_query, $load_card_type, $load_card_term, $additional_filter,  $label, $label_no_more, $classes;
...
$query_params = '?post_count='.$the_query->post_count.'&load_posts='.$load_posts.'&search='.$query_search.'&post_types='.$post_types.'&load_card_type='.$load_card_type.'&load_card_term='.$load_card_term.'&query_params='.$query_params.'&additional_filter='.$additional_filter;

if($the_query->post_count < $the_query->found_posts) {
?> 
<div class="d-flex justify-content-center mt-4" id="load-more-btn">
    <?php if(strtolower(get_parent_template()) === 'servizi.php') {
        ?><button type="button"
