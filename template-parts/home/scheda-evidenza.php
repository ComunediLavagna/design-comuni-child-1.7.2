...
<div class="card card-teaser card-teaser-image card-flex no-after rounded shadow-sm border border-light mb-0">
    <div class="card-image-wrapper with-read-more">
        <div class="card-body p-3 u-grey-light">
            ...
            <h3 class="card-title h5 text-paragraph-medium u-grey-light"><?php echo $post->post_title ?></h3>
            <p class="card-text text-paragraph-card u-grey-light m-0" style="margin-bottom: 40px!important;"><?php echo $descrizione_breve ?></p>
        </div>
        <div class="card-image card-image-rounded pb-5">            
            <?php sts_get_img_by_id( attachment_url_to_postid( $img ), '', '', '', array(280,280) /*'thumbnail'*/ ); /* dci_get_img($img); */ ?>
        </div>
    </div>
    <a
...
<?php } else { ?>
    <div class="card card-teaser no-after rounded border border-light shadow-sm mb-0">
        ...
        <h3 class="card-title h5 text-paragraph-medium u-grey-light">
            <?php echo $post->post_title ?>
        </h3>
        <p class="card-text text-paragraph-card u-grey-light m-0">
            <?php echo $descrizione_breve ?>
        </p>
        </div>
        <a class="read-more" href="<?php echo get_permalink($post->ID); ?>" aria-label="Vai alla pagina <?php echo esc_html( $post->post_title ) ?>" title="Vai alla pagina <?php echo esc_html( $post->post_title ) ?>"
        ><span class="text">Vai alla pagina</span>
        <svg class="icon ms-0">
            <use
