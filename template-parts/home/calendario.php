<?php
$calendario = dci_get_calendar(16); // 2 settimane
		...
		<div class="container">
			...
			<div class="row row-calendar">
				<?php if ($total_eventi > 0) { ?>
													...
													<div class="card-body">
														...
														if (is_array($eventi) && count($eventi)) {
															foreach ($eventi as $evento) {
																$imgid = dci_get_meta('immagine_id', '_dci_evento_', $evento['id']);
															#	$img = wp_get_attachment_image_url($imgid); // resized image, default size is 'thumbnail'
															#	if (!$img)
															#		$img = dci_get_meta('immagine', '_dci_evento_', $evento['id']); // original image (larger!)
														?>
																<p class="card-text px-2 pb-10 mb-10 d-flex">
																	<?php if ($imgid) sts_get_img_by_id($imgid, 'me-3 rounded', 150, 150, 'thumbnail'); ?>
					...
					<div class="my-5 justify-content-md-center">
					    <a role="button" class="btn btn-outline-primary d-inline-block read-more pb-3" href="<?php echo dci_get_template_page_url("page-templates/eventi.php"); ?>">
						Tutti gli eventi
						    <svg class="icon">
