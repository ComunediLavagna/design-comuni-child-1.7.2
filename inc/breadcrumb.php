 ...
 * @return void
 */
function breadcrumb_trail_2( $args = array() ) {

	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new Breadcrumb_Trail_2( $args );
...
class Breadcrumb_Trail_2 {
	...
	protected function add_items() {
		...
		if (get_post_type() == 'persona_pubblica') {
			...
			$this->items[] =  "<a href='".home_url("persona_pubblica")."'>".__("Persone pubbliche", "design_comuni_italia")."</a>";
			$this->items[] = get_the_title();
			return;
		}

		if (get_post_type() == 'unita_organizzativa') {
			#$this->items[] =  "<a href='".home_url("unita_organizzativa")."'>".__("Unità organizzativa", "design_comuni_italia")."</a>";
			...
			if($terms){
			  ...
			  foreach ($terms as $term) {
				  // Aree amministrative:
				  if (
					$term->slug == 'area' || $term->slug == 'servizio' || $term->slug == 'ufficio' 
				    ) {
					$this->items[] =  "<a href='".home_url("amministrazione")."'>".__("Amministrazione", "design_comuni_italia")."</a>";
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, 'tipi_unita_organizzativa' ) ), ucfirst($term->name ));
					$found = true;
					  break;
				  }
				  // Organi di governo:
				  if (
					$term->slug == 'commissione' || $term->slug == 'consiglio-comunale' || $term->slug == 'giunta-comunale'
				    ) {
					$this->items[] =  "<a href='".home_url("amministrazione")."'>".__("Amministrazione", "design_comuni_italia")."</a>";
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, 'tipi_unita_organizzativa' ) ), 'Organi di governo');
					$found = true;
					  break;
				  }
			  }
		...
		if (get_post_type() == 'notizia') {
			$this->items[] =  "<a href='".home_url("novita")."'>".__("Novita", "design_comuni_italia")."</a>";
